...
final class VergaGoldEngine
{
    ...
    /** Ca „dollar” la Millionaire Crown: simbolul 10 pe linie. */
    private const TEN_TOTAL_BET_PAY = [
        3 => 2,
        4 => 20,
        5 => 100,
    ];
    ...
    private static function mechanicalReelStrips(): array
    {
        ...
        $path = dirname(__DIR__, 3) . DIRECTORY_SEPARATOR . 'games' . DIRECTORY_SEPARATOR . 'verga-gold' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'reel-strips.json';
        if (!is_readable($path)) {
            throw new RuntimeException('Lipsa fisierului reel-strips.json pentru Brain Way.');
        }

        $decoded = json_decode((string) file_get_contents($path), true);
        if (!is_array($decoded) || !is_array($decoded['strips'] ?? null) || count($decoded['strips']) !== 5) {
            throw new RuntimeException('Fisier reel-strips.json invalid pentru Brain Way.');
        }
    ...
    private static function evaluatePayline(array $board, array $lineRows, float $totalBet): ?array
    {
        ...
        if ($target === 'ten') {
            $multiplier = self::TEN_TOTAL_BET_PAY[$matched] ?? 0;
            if ($multiplier <= 0) {
                return null;
            }
            $payout = round($totalBet * $multiplier, 2);
